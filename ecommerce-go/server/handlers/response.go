...
func SuccessArrResidentialasRespond(fields []*models.Residential, writer http.ResponseWriter) {
	// var fields["status"] := "success"
	_, err := json.Marshal(fields)
	type data struct {
		Residential []*models.Residential `json:"data"`
		Statuscode  int                   `json:"status"`
		Message     string                `json:"msg"`
	}
	temp := &data{Residential: fields, Statuscode: 200, Message: "success"}
	if err != nil {
		ServerErrResponse(err.Error(), writer)
	}
...
func SuccessResidentialasRespond(field *models.Residential, writer http.ResponseWriter) {
	// var fields["status"] := "success"
	_, err := json.Marshal(field)
	type data struct {
		Residential *models.Residential `json:"data"`
		Statuscode  int                 `json:"status"`
		Message     string              `json:"msg"`
	}
	temp := &data{Residential: field, Statuscode: 200, Message: "success"}
	if err != nil {
		ServerErrResponse(err.Error(), writer)
	}
